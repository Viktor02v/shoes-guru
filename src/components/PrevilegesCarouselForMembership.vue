...
import SliderForPrevilegesCarouselMembership from './SliderForPrevilegesCarouselMembership.vue';
...
<template>
	<div class="w-full">
		...
		<Carousel
			:class="currentSlide !== 0 ? 'pl-0 transition easy-out duration-200 ' : 'pl-5 transition easy-in duration-200 '"
			class="pr-10" ref="carousel" v-model="currentSlide" :items-to-show="3" :items-to-scroll="1" :transition="800"
			snapAlign="start">
			<Slide v-for="slide in data" :key="slide" class="flex items-baseline">
				<SliderForPrevilegesCarouselMembership :slide="slide" />
			</Slide>
		</Carousel>
	</div>
</template>